...
export async function POST(request: NextRequest) {
  try {
    const { action, txBytes, userAddress, signature, digest } =
      await request.json();
    ...
    if (action === "create") {
      ...
      const sponsoredTx = await enoki.createSponsoredTransaction({
        ...
        allowedMoveCallTargets: [
          `${process.env.NEXT_PUBLIC_PACKAGE_ID}::tip_jar_live::tip`,
        ],
    ...
    return NextResponse.json(
      {
        ...
        message:
          "Transaction sponsorship failed. The transaction may not be whitelisted or there may be a configuration issue.",
      },
      { status: 500 }
    );
  }
